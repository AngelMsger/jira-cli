...
type LoadOptions struct {
	...
	// over JIRA_CONTEXT and the file's current_context.
	Context string
}
...
// JIRA_CONTEXT env var, the file's current_context, the sole context,
...
func selectContext(f File, flagCtx, envCtx string) (string, string, error) {
	...
	case envCtx != "":
		return pick(envCtx, "JIRA_CONTEXT", ContextSourceEnv)
...
func unknownContextHint(name string, available []string) string {
	...
	return "Run `jira-cli config get-contexts` to list defined contexts."
}
