...
func TestSetupTargetsNamedContextAndExcludesPersonalEnvironment(t *testing.T) {
	...
	t.Setenv("JIRA_CONTEXT", "not-defined")
	t.Setenv("JIRA_FORMAT", "ndjson")
	t.Setenv("JIRA_USERNAME", "InjectedUser")
	t.Setenv("JIRA_API_TOKEN", "must-not-be-copied")
	dotenv := filepath.Join(dir, "presets.env")
	if err := os.WriteFile(dotenv, []byte("JIRA_SERVER=https://dotenv.example.test/deploy\nJIRA_CREDENTIAL_URL=https://help.example.test/new\nJIRA_API_TOKEN=dotenv-secret\nJIRA_USERNAME=dotenv-user\n"), 0600); err != nil {
		t.Fatal(err)
	}
	...
	t.Setenv("JIRA_SERVER", "https://env.example.test")
	t.Setenv("JIRA_AUTH_SCHEME", "basic")
	got, err = Load(opt)
	if err != nil {
		t.Fatal(err)
	}
...
func TestPlanServiceContextConflictIdempotencyAndPreservation(t *testing.T) {
	...
	dir := t.TempDir()
	target := setupFixture()
	file := File{CurrentContext: "other", Contexts: []NamedContext{{Name: "other", BaseURL: "https://other.example.test"}, target}, Defaults: Defaults{ReadOnly: true, Project: "KEEP"}}
	if err := WriteFile(dir, file); err != nil {
		t.Fatal(err)
	}
