...
// Flavor values for the Jira backend.
const (
	FlavorAuto       = "auto"
	FlavorCloud      = "cloud"
...
const (
	...
	ContextSourceEnv     = "env"             // JIRA_CONTEXT
...
// NamedContext is one named Jira server profile inside the config file.
...
type Defaults struct {
	...
	// Project is the default project key, used when a command that takes
	// --project is invoked without one.
	Project string `yaml:"project,omitempty"`
	// ReadOnly blocks every mutating client method. Settable from the config
	// file, from JIRA_CLI_READ_ONLY, or temporarily overridden via
	// --allow-writes.
...
// this invocation (via --use-context or JIRA_CONTEXT) rather than fallen
