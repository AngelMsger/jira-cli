...
import (
	...
	"github.com/angelmsger/jira-cli/internal/cliflags"
...
func newRootCmdWithState() (*cobra.Command, *appState) {
	...
	root := &cobra.Command{
		...
		Short: "Drive Jira issue-tracking workflows from the command line, built for coding agents",
		Long: "jira-cli reads and searches Jira issues via JQL, creates and edits\n" +
			"issues, moves them through workflow transitions, and manages comments.\n" +
			"It supports Jira Cloud and Data Center / Server, and emits agent-friendly\n" +
	...
	pf.StringVar(&state.gflags.configPath, "config", "", "config directory (default ~/.angelmsger/jira)")
	...
	pf.BoolVar(&state.gflags.allowWrites, "allow-writes", false,
		"override read-only mode (defaults.read_only / JIRA_CLI_READ_ONLY) for this invocation")

	root.SetFlagErrorFunc(func(_ *cobra.Command, err error) error {
		return cerrors.Wrap(err, cerrors.CategoryUsage, "BAD_FLAG", err.Error())
	})
	// `jira-cli --version` prints the same line as the `version` command.
