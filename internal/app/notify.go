...
const noUpdateNotifierEnv = "JIRA_CLI_NO_UPDATE_NOTIFIER"

// updateCommandHint is the command the notice tells the user/agent to run to
// upgrade. There is no `jira-cli update` subcommand, so it points at the
// npm package the binary is distributed through.
const updateCommandHint = "npm install -g @angelmsger/jira-cli@latest"
