...
const (
	...
	envSkillLoaded = "JIRA_CLI_SKILL"
	// envNoSkillHint opts out of the nudge entirely.
	envNoSkillHint = "JIRA_CLI_NO_SKILL_HINT"
)
...
func maybeSkillHint(cmd *cobra.Command) {
	...
	message := "The companion Skill 'jira' is the source of truth for driving this CLI " +
		"(usage recipes, safety modes, env setup). If you are an agent, load it before composing commands."
	var nextSteps []string
	if load.Loaded {
		message = "The loaded companion Skill 'jira' does not match this CLI. Refresh it before composing commands."
		nextSteps = []string{
