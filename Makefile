BINARY      := jira-cli
PKG         := github.com/angelmsger/jira-cli
...
build:
	CGO_ENABLED=0 go build -trimpath -ldflags "$(LDFLAGS)" -o bin/$(BINARY) ./cmd/jira-cli
...
e2e-live: build
	JIRA_E2E_LIVE=1 ./scripts/e2e.sh
...
install-skill:
	mkdir -p $(SKILL_DIR)
	rm -rf $(SKILL_DIR)/jira
	cp -R skills/jira $(SKILL_DIR)/jira
	@echo "installed skill -> $(SKILL_DIR)/jira"
