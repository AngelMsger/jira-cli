...
`jira-cli` is distributed from **GitHub Releases**. Everything else — the
...
- The repository is public at `github.com/AngelMsger/jira-cli`.
- The npm account owns the `@angelmsger` scope; `@angelmsger/jira-cli`
...
- **npm publishing uses trusted publishing (OIDC)** — no long-lived token, no
  ...
  npmjs.com → the `@angelmsger/jira-cli` **package** → Settings → Trusted
...
3. **`repository.url` casing must match the GitHub repo exactly.** Provenance
   ...
   repo reported by the OIDC provenance (`AngelMsger/jira-cli`); a casing
...
5. sets the npm package version to the tag (minus the `v`) and runs
   `npm publish --access public` for `@angelmsger/jira-cli`, skipping the
...
<https://angelmsger.github.io/jira-cli/>.
...
```
jira-cli-darwin-amd64    jira-cli-linux-amd64    jira-cli-windows-amd64.exe
jira-cli-darwin-arm64    jira-cli-linux-arm64    jira-cli-windows-arm64.exe
checksums.txt
```

Download URL pattern:
`https://github.com/angelmsger/jira-cli/releases/download/v<version>/<asset>`

## Companion Skill

The `jira` Skill is **embedded into the binary** at build time
(`//go:embed skills/jira`, see `assets.go`), so every release ships a
Skill that matches the CLI version; users deploy it with `jira-cli skill
...
`skills/jira/SKILL.md`. Bump it whenever the Skill or its `references/`
